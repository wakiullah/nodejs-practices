...
lib.update = (folderName, fileName, data, callback) => {
    ...
    fs.writeFile(lib.baseDir + folderName + '/' + fileName + '.json', JSON.stringify(data), { flag: 'r+' }, (err) => {
        if (!err) {
    ...
    //                                 callback(false, 'File updated successfully')
    //                             } else {
    //                                 callback(err, 'Failed to close file')
    //                             }
    //                         })
    //                     } else {
    //                         callback(err, 'Failed to write file')
    //                     }
    //                 })
    //             } else {
    ...
    //         })
    //     } else {
    //         callback(err, 'Failed to open file for update')
    //     }
    // })
}
...
lib.delete = (folderName, fileName, callback) => {
    fs.unlink(lib.baseDir + folderName + '/' + fileName + '.json', (err) => {
        if (!err) {
            ...
            callback(err, 'Failed to delete file')
        }
    }
    )
}
